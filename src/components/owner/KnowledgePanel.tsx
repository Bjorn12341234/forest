    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-white/60 border border-[#2D6A4F]/20 rounded-sm p-3">
          <span className="text-[#3D2B1F]/60 text-xs uppercase tracking-wider">Skogskunskap</span>
          <AnimatedNumber value={kunskap} className="text-lg text-[#2D6A4F]" />
        </div>
        <div className="bg-white/60 border border-[#2D6A4F]/20 rounded-sm p-3">
          <span className="text-[#3D2B1F]/60 text-xs uppercase tracking-wider">Resiliens</span>
          <AnimatedNumber value={resiliens} className={`text-lg ${resiliens > 50 ? 'text-[#2D6A4F]' : resiliens > 20 ? 'text-[#B8860B]' : 'text-[#CC2222]'}`} />
        </div>
      </div>
      ...
      <div className="bg-white/60 border border-[#2D6A4F]/20 rounded-sm p-3">
        ...
        {currentThreshold && (
          <div className="mb-2">
            <span className="text-sm font-medium text-[#2D6A4F]">{currentThreshold.label}</span>
            <p className="text-xs text-[#3D2B1F]/50 italic mt-0.5">{currentThreshold.description}</p>
          </div>
        )}

        {nextThreshold && (
          <div className="w-full h-2 bg-[#3D2B1F]/10 rounded-sm overflow-hidden mb-2">
            <div
              className="h-full bg-[#2D6A4F]/60 transition-all duration-500"
            ...
            return (
              <div
                ...
                className={`bg-white/60 border rounded-sm p-3 select-none
                  ${canAfford ? 'border-[#2D6A4F]/50 cursor-pointer hover:border-[#2D6A4F]' : 'border-[#3D2B1F]/10 opacity-50'}`}
                onClick={() => {
                  if (canAfford) {
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-sm font-medium text-[#3D2B1F]">{activity.name}</span>
                    <p className="text-xs text-[#3D2B1F]/50 leading-relaxed">{activity.description}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    ...
                    <div className="text-xs text-[#2D6A4F] font-numbers">+{activity.kunskapReward} kunskap</div>
                  </div>
                </div>
              </div>
            )
