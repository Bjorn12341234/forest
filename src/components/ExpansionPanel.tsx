...
type MapView = 'world' | 'solar' | 'galaxy' | 'multiverse'

function getMapView(phase: number): MapView {
  if (phase <= 7) return 'world'
  if (phase <= 9) return 'solar'
  if (phase <= 10) return 'galaxy'
  return 'multiverse'
}
...
function GalaxySVG() {
  // Spiral arm approximation with paths
  return (
    <svg viewBox="0 0 100 100" className="w-full h-full" preserveAspectRatio="xMidYMid meet">
      {/* Center glow */}
  ...
  return (
    <div className="flex flex-col gap-4 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[0.6rem] text-text-muted">
            {mapView === 'world' && 'Utvidga verksamheten till nya territorier.'}
            {mapView === 'solar' && 'Interplanetär skogsbruksindustri.'}
            {mapView === 'galaxy' && 'Galaktisk expansion av skogsimperiet.'}
            {mapView === 'multiverse' && 'Skogsbruk \u00f6ver alla verkligheter.'}
          </p>
        </div>
      ...
      <GlassCard padding="sm" className="relative overflow-hidden">
        <div className="relative w-full" style={{ paddingBottom: mapView === 'world' ? '56%' : '70%' }}>
          {/* SVG Background */}
